...
// Separate isolated trial so a broken DMA import cannot erase a working CPU-buffer encoder result.
std::optional<std::string> probe_zero_copy(const Device &device,
                                           const EncoderBinding &binding,
