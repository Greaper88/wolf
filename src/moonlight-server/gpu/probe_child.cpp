...
int probe_child(int argc, char **argv) {
  ...
  for (const auto &device : discover()) {
    ...
    if (!bind_zero_copy(encoder))
      return 2;
    std::ostringstream output;
    output << "WOLF_GPU_V2 " << encoder.factory << " " << encoder.plugin << " "
           << (encoder.cuda_device ? std::to_string(*encoder.cuda_device) : "-1") << " " << encoder.converter << " "
           << std::quoted(encoder.producer_caps) << "\n";
    auto reply = output.str();
    if (reply.size() >= 1024)
      return 2;
